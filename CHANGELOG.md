...
- **js/activityAdmin.js** — เพิ่ม validation ก่อนสร้างกิจกรรม (ห้ามชื่อ/วันที่ว่าง), เพิ่มตารางจัดการกิจกรรมที่มีอยู่ (เรียก `closeEvent`/`reopenEvent` ที่ backend มีแต่ไม่มี UI เรียกใช้มาก่อน) พร้อมลิงก์ไปหน้ารายงาน
- **js/api.js** — เพิ่ม try/catch ดัก network error ให้คืนค่า shape เดียวกับที่ทุกหน้าเช็คอยู่แล้ว (`success:false`), เพิ่ม helper กลาง `escapeHtml()` และ `getEventInfo()`
