...
## เพิ่มฟีเจอร์ใหม่ (รอบล่าสุด)

- **backend/Reports.gs** — `getEventSummary` เพิ่ม `submittedDepts` (แผนกที่ส่งยอดแล้ว) และ `pendingDepts` (แผนกที่ยังไม่ส่ง) โดยเทียบรายชื่อแผนกทั้งหมดจากชีต Departments กับชีต EventSubmit ของ eventId นั้น
- **pages/activity-report.html** + **js/activityReport.js** — เพิ่มตาราง "สถานะการส่งยอดของแผนก" แสดงทุกแผนกพร้อม badge ส่งแล้ว/ยังไม่ส่ง ในหน้ารายงาน
- **js/api.js** — เพิ่ม loading bar บาง ๆ ที่ขอบบนของหน้า แสดงอัตโนมัติทุกครั้งที่มีการเรียก `api()` (ไม่ต้องแก้ทุกหน้าทีละหน้า เพราะทุกหน้าเรียกผ่านฟังก์ชันนี้อยู่แล้ว) เป็นแบบ indeterminate (ไม่รู้ % จริงเพราะเป็น fetch เดียวจบ ไม่มี progress event ให้ใช้) ไล่ไปประมาณ 85% ระหว่างรอ แล้ววิ่งไป 100% และจางหายตอนได้ผลลัพธ์ รองรับกรณีมีหลาย request พร้อมกัน (เช่นหน้าฟอร์มที่โหลด event info + แผนกพร้อมกัน) ด้วยตัวนับ ไม่หายก่อนที่ request สุดท้ายจะเสร็จ
